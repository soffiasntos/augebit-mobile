...
app.get('/requisicoes/estatisticas', (req, res) => {
  ...
  req.dbConnection.query(query, (err, results) => {
    if (err) {
      console.error('Erro ao consultar estatísticas de requisições:', err);
      return res.status(500).json({
        success: false,
        error: 'Erro ao consultar estatísticas de requisições',
        details: err.message
      });
    }
    
    console.log(`Encontradas estatísticas para ${results.length} meses`);
    console.log('Dados:', results);
    
    // Inverter ordem para mostrar do mais antigo para o mais recente
    const estatisticas = results.reverse();
...
// Rota alternativa caso não tenha created_at
app.get('/requisicoes/estatisticas-alt', (req, res) => {
  console.log('Buscando estatísticas de requisições (método alternativo)...');
  ...
  req.dbConnection.query(checkColumnsQuery, (err, columns) => {
    if (err) {
    ...
    const hasDataRequisicao = columns.some(c => c.COLUMN_NAME === 'data_requisicao');
    const hasData = columns.some(c => c.COLUMN_NAME === 'data');
    
    let dateColumn = 'NOW()'; // fallback
    if (hasCreatedAt) dateColumn = 'created_at';
    else if (hasDataRequisicao) dateColumn = 'data_requisicao';
    else if (hasData) dateColumn = 'data';
    
    const query = `
      SELECT 
        DATE_FORMAT(${dateColumn}, '%Y-%m') as mes,
        COUNT(*) as total,
        SUM(CASE WHEN status = 'atendida' THEN 1 ELSE 0 END) as atendidas,
        SUM(CASE WHEN status = 'pendente' THEN 1 ELSE 0 END) as pendentes,
        SUM(CASE WHEN status = 'atrasada' THEN 1 ELSE 0 END) as atrasadas
      FROM requisicoes 
      ${dateColumn !== 'NOW()' ? `WHERE ${dateColumn} >= DATE_SUB(NOW(), INTERVAL 6 MONTH)` : ''}
